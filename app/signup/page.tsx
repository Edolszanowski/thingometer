...
function SignUpPageContent() {
  ...
  useEffect(() => {
    ...
    if (eventIdParam) {
      ...
      if (!isNaN(parsedEventId)) {
        ...
        // Fetch event data for title
        fetch(`/api/events`)
          .then((res) => res.json())
          .then((events) => {
            const event = events.find((e: any) => e.id === parsedEventId)
            if (event) {
              setEventData({ name: event.name, city: event.city })
            }
          })
          .catch(() => {
            // Ignore errors
    ...
    // Check if signups are locked (but coordinators can bypass)
    fetch("/api/coordinator/settings")
      .then((res) => res.json())
      .then((data) => {
        // Coordinators can always add entries
        setSignupLocked(coordUser ? false : (data.signupLocked || false))
      })
      .catch(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const driverEmailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
      if (!driverEmailRegex.test(formData.driverEmail.trim())) {
      ...
      // Use "Other" value if selected, otherwise use selected type
      const finalTypeOfEntry = formData.typeOfEntry === "Other" 
      ...
      const response = await fetch("/api/entries", {
        ...
        body: JSON.stringify({
          ...
          driverFirstName: formData.driverFirstName.trim(),
          driverLastName: formData.driverLastName.trim(),
          driverPhone: formData.driverPhone.trim(),
          driverEmail: formData.driverEmail.trim(),
          entryName: formData.entryName.trim() || null,
          floatDescription: formData.floatDescription.trim(),
          entryLength: formData.entryLength.trim() || null,
  ...
  return (
    <div className="min-h-screen p-4 bg-background">
      <div className="container mx-auto max-w-2xl">
        {/* Coordinator banner when signups are locked */}
        {isCoordinatorUser && (
            ...
            <div className="pt-4">
              <Button
                ...
                className={`w-full text-white ${
                  isCoordinatorUser && formData.autoApprove 
