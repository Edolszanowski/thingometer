...
export default function JudgeLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        // Store judge session cookie with city
        document.cookie = `judge-auth=${password}; path=/; max-age=3600`
        document.cookie = `judge-city-id=${cityId}; path=/; max-age=3600`
      ...
      <div className="max-w-md w-full space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">
            Select your city and enter the judge password
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? "Verifying..." : "Access Judge Portal"}
          </Button>
        </form>
      </div>
    </div>
